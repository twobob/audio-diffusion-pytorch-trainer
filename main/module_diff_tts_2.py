...
class Model(pl.LightningModule):
    ...
    def get_text_embedding(self, texts: List[str]) -> Tensor:
        ...
        # mask = encoded["attention_mask"].to(self.device).bool(), mask=mask
        # Compute embedding
        embedding = self.text_embedding(tokens)
    ...
    def training_step(self, batch, batch_idx):
        ...
        loss = self.model(x, embedding=embedding)
...
class SampleLogger(Callback):
    ...
    def log_sample(self, trainer, pl_module, batch):
        ...
        log_wandb_embeddings(logger=wandb_logger, id="text", embeddings=embedding)
