...
class Model(pl.LightningModule):
    def __init__(
        ...
        num_artists: int,
        num_genres: int,
        artist_genre_embedding_dim: int = 300,
    ):
        super().__init__()
        self.lr = lr
        ...
        # num_category_types = num_embeddings (2: artists and genres)
        # num_categories = embedding_dim (4 genres and artists per song)

        self.artist_embedding = nn.Embedding(num_artists, artist_genre_embedding_dim)
        self.genre_embedding = nn.Embedding(num_genres, artist_genre_embedding_dim)
    ...
    def training_step(self, batch, batch_idx):
        ...
        embedding = self.artist_embedding(info[:, 0])
        embedding += self.genre_embedding(info[:, 1])
...
def log_wandb_audio_batch(
    logger: WandbLogger, id: str, samples: Tensor, sampling_rate: int, caption: str = "", artists=None, genres=None
):
    ...
    logger.log(
        {
            f"sample_{idx}_{id}": wandb.Audio(
                samples[idx],
                caption=f"{caption} Artist={artists[idx].tolist()} Genre={genres[idx].tolist()}" if artists is not None and genres is not None else caption,
...
class SampleLogger(Callback):
    ...
    def log_sample(self, trainer, pl_module, batch):
        ...
        embedding = pl_module.artist_embedding(info[:, 0])
        embedding += pl_module.genre_embedding(info[:, 1])
